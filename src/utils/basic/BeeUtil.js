const BeeUtil = {
    ...
    StringUtils: {
        ...
        isNest: function (rule, str) {
            ...
            var stack = [];
            for (var i = 0; i < len; i++) {
                if (BeeUtil.ArrayUtils.inArray(keys, chs[i])) {
                    stack.push(rule[chs[i]]);
                } else {
                    if (chs[i] === stack[stack.length - 1]) {
                        stack.pop();
                    } else if (BeeUtil.ArrayUtils.inArray(values, chs[i])) {
                        return false;
                    }
                }
            }
    ...
    IdCardUtils: {
        ...
        getPersonInfo18: function (idCard) {
            ...
            var address = '';
            var sex = '';
            address = BeeUtil.areas[idCard.substr(0, 2) + '0000'] + ' ' + BeeUtil.areas[idCard.substr(0, 4) + '00'] + ' ' + BeeUtil.areas[idCard.substr(0, 6)];
    ...
    DateUtils: {
        ...
        formatDateTimePatternShow(dateTime) {
            return BeeUtil.DateUtils.format(dateTime,'yyyy-MM-dd hh:mm:ss');
        },
        formatDatePatternShow(date) {
           return BeeUtil.DateUtils.format(date,'yyyy-MM-dd');
        },
        /**
         * 日期时间格式化
        ...
        format: function (dateTime, pattern) {
            ...
            if (BeeUtil.StringUtils.isBlank(pattern)) {
                return date.toLocaleString();
            }
            return pattern.replace(/([a-z])\1*/ig, function (matchStr, group1) {
                var replacement = "";
                switch (group1) {
                    case BeeUtil.DateUtils.patterns.PATTERN_ERA: //G
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_WEEK_YEAR: //Y
                    case BeeUtil.DateUtils.patterns.PATTERN_YEAR: //y
                        replacement = date.getFullYear();
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_MONTH: //M
                        var month = date.getMonth() + 1;
                        replacement = (month < 10 && matchStr.length >= 2) ? "0" + month : month;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_DAY_OF_MONTH: //d
                        var days = date.getDate();
                        replacement = (days < 10 && matchStr.length >= 2) ? "0" + days : days;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_HOUR_OF_DAY1: //k(1~24)
                        var hours24 = date.getHours();
                        replacement = hours24;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_HOUR_OF_DAY0: //H(0~23)
                        var hours24 = date.getHours();
                        replacement = (hours24 < 10 && matchStr.length >= 2) ? "0" + hours24 : hours24;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_MINUTE: //m
                        var minutes = date.getMinutes();
                        replacement = (minutes < 10 && matchStr.length >= 2) ? "0" + minutes : minutes;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_SECOND: //s
                        var seconds = date.getSeconds();
                        replacement = (seconds < 10 && matchStr.length >= 2) ? "0" + seconds : seconds;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_MILLISECOND: //S
                        var milliSeconds = date.getMilliseconds();
                        replacement = milliSeconds;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_DAY_OF_WEEK: //E
                        var day = date.getDay();
                        replacement = BeeUtil.DateUtils.week['ch'][day];
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_DAY_OF_YEAR: //D
                        replacement = BeeUtil.DateUtils.dayOfTheYear(date);
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_DAY_OF_WEEK_IN_MONTH: //F
                        var days = date.getDate();
                        replacement = Math.floor(days / 7);
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_WEEK_OF_YEAR: //w
                        var days = BeeUtil.DateUtils.dayOfTheYear(date);
                        replacement = Math.ceil(days / 7);
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_WEEK_OF_MONTH: //W
                        var days = date.getDate();
                        replacement = Math.ceil(days / 7);
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_AM_PM: //a
                        var hours24 = date.getHours();
                        replacement = hours24 < 12 ? "\u4e0a\u5348" : "\u4e0b\u5348";
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_HOUR1: //h(1~12)
                        var hours12 = date.getHours() % 12 || 12; //0转为12
                        replacement = (hours12 < 10 && matchStr.length >= 2) ? "0" + hours12 : hours12;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_HOUR0: //K(0~11)
                        var hours12 = date.getHours() % 12;
                        replacement = hours12;
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_ZONE_NAME: //z
                        replacement = BeeUtil.DateUtils.getZoneNameValue(date)['name'];
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_ZONE_VALUE: //Z
                        replacement = BeeUtil.DateUtils.getZoneNameValue(date)['value'];
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_ISO_DAY_OF_WEEK: //u
                        break;
                    case BeeUtil.DateUtils.patterns.PATTERN_ISO_ZONE: //X
                        break;
                    default:
                        break;
                }
    ...
    ElementaryArithmeticUtils: {
        ...
        infixToPostfixExpression: function (expression) {
            expression = BeeUtil.StringUtils.deleteWhitespace(expression);
            ...
            for (var i = 0; i < size; i++) {
                if (BeeUtil.StringUtils.isNumeric(elementArr[i])) {
                    //如果是数值
        ...
        calculateByPrefixExpression: function (expression) {
            ...
            for (var i = size - 1; i >= 0; i--) {
                if (BeeUtil.StringUtils.isNumeric(elementArr[i])) {
                    ...
                    switch (elementArr[i]) {
                        case '+':
                            result = BeeUtil.ElementaryArithmeticUtils.add(operand1, operand2);
                            break;
                        case '-':
                            result = BeeUtil.ElementaryArithmeticUtils.subtract(operand1, operand2);
                            break;
                        case '×':
                        case '*':
                            result = BeeUtil.ElementaryArithmeticUtils.multiply(operand1, operand2);
                            break;
                        case '\/':
                        case '÷':
                            result = BeeUtil.ElementaryArithmeticUtils.divide(operand1, operand2);
                            break;
                        default:
                            result = '';
        ...
        calculateByPostfixExpression: function (expression) {
            ...
            for (var i = 0; i < size; i++) {
                if (BeeUtil.StringUtils.isNumeric(elementArr[i])) {
                    ...
                    switch (elementArr[i]) {
                        case '+':
                            result = BeeUtil.ElementaryArithmeticUtils.add(operand2, operand1);
                            break;
                        case '-':
                            result = BeeUtil.ElementaryArithmeticUtils.subtract(operand2, operand1);
                            break;
                        case '×':
                        case '*':
                            result = BeeUtil.ElementaryArithmeticUtils.multiply(operand2, operand1);
                            break;
                        case '\/':
                        case '÷':
                            result = BeeUtil.ElementaryArithmeticUtils.divide(operand2, operand1);
                            break;
                        default:
                            result = '';
                            alert("The operator 【" + elementArr[i] + "】 is not legal");
