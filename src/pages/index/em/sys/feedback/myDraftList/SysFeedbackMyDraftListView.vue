...
<script>
    import {SysFeedbackMyDraftListApi} from './SysFeedbackMyDraftListApi.js'
    ...
    export default {
        ...
        methods: {
            ...
            dealGetAllTypeTree() {    //取得所有的 类型-树
                var _this = this;
                SysFeedbackMyDraftListApi.queryTypeTree().then((res) => {
                    if (res.success) {
            ...
            dealBatchDeleteByIds() {  //批量删除
                ...
                SysFeedbackMyDraftListApi.batchDeleteByIds(delIds).then((res) => {
            ...
            dealDelOneRowById(delId) {   //根据id 删除
                var _this = this;
                SysFeedbackMyDraftListApi.deleteById(delId).then((res) => {
                    if (res.success) {  //已经有对错误进行预处理
                        _this.$message.success(res.msg);
                        _this.mixin_invokeQuery(_this); //表格重新搜索
                    }
            ...
            handleSearchFormQuery(e, values) {   //带查询条件 检索【系统反馈】列表
                ...
                SysFeedbackMyDraftListApi.querySelfDraftDtoPage(values, _this.tableConf).then((res) => {
